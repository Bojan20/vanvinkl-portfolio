...
interface SlotMachine3DSimpleProps {
...
export function SlotMachine3DSimple({
  position,
  rotation = [0, 0, 0],
  machineId,
  isActive = false,
  onInteract
}: SlotMachine3DSimpleProps) {
  ...
  return (
    <group ref={groupRef} position={position} rotation={rotation}>
      ...
      {/* Spin Button */}
      <Float speed={isHovered ? 4 : 2} rotationIntensity={0.3} floatIntensity={0.3}>
        <Cylinder
      ...
      <Float speed={1} rotationIntensity={0.2} floatIntensity={0.4}>
        <group position={[0, 2, 0]}>
          <RoundedBox args={[1.6, 0.8, 0.2]} radius={0.05} castShadow>
            {goldMaterial}
          </RoundedBox>
          ...
          {[
            ...
            <Sphere key={i} args={[0.08, 16, 16]} position={pos as [number, number, number]}>
              <meshBasicMaterial color={isActive ? "#FFD700" : "#664400"} />
            </Sphere>
          ))}
      ...
      {/* Side panels */}
      {[-0.95, 0.95].map((x, i) => (
        <RoundedBox key={i} args={[0.1, 2.8, 1]} radius={0.02} position={[x, 0.2, 0]}>
          <meshStandardMaterial
            color="#FFD700"
            metalness={0.9}
            roughness={0.2}
