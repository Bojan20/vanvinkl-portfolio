...
function NeonStrip({ color, position, size, pulse = false, audioReactive = false }: {
  color: string, position: [number, number, number], size: [number, number, number], intensity?: number, pulse?: boolean, audioReactive?: boolean
}) {
  const mat = useMemo(() => {
    if (!pulse && !audioReactive) {
  ...
  }, [color, pulse, audioReactive])

  // Update time and bass uniforms
  useFrame((state) => {
    if ((pulse || audioReactive) && mat instanceof THREE.ShaderMaterial) {
...
export function CasinoScene({ onShowModal, onSlotSpin, introActive = false, slotOpen = false }: CasinoSceneProps) {
  ...
  return (
    <>
      ...
      {/* Wall neon accents - AUDIO REACTIVE */}
      <NeonStrip color={COLORS.magenta} position={[0, 1, -11.7]} size={[65, 0.06, 0.06]} audioReactive />
      <NeonStrip color={COLORS.cyan} position={[0, 8.5, -11.7]} size={[65, 0.06, 0.06]} audioReactive />
      ...
      {/* Ceiling neon grid - AUDIO REACTIVE */}
      <NeonStrip color={COLORS.magenta} position={[-15, 9.3, 10]} size={[0.04, 0.04, 55]} audioReactive />
      <NeonStrip color={COLORS.cyan} position={[15, 9.3, 10]} size={[0.04, 0.04, 55]} audioReactive />
