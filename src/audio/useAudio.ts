...
export function startAmbientMusic(
  soundId: 'lounge' | 'casinoHum' | 'neonBuzz' = 'lounge',
  fadeTime = 1.0,
  volume = 0.3
): void {
  audioSystem.startAmbientMusic(soundId, fadeTime, volume)
}

// Stop ambient music with fade out
export function stopAmbientMusic(fadeTime = 1.0): void {
  audioSystem.stopAmbientMusic(fadeTime)
}

// Crossfade to different ambient track
export function crossfadeAmbientMusic(
  soundId: 'lounge' | 'casinoHum' | 'neonBuzz',
  fadeTime = 2.0,
  volume = 0.3
): void {
  audioSystem.crossfadeAmbientMusic(soundId, fadeTime, volume)
}

// Set ambient music volume
export function setAmbientVolume(volume: number, fadeTime = 0.3): void {
  audioSystem.setAmbientVolume(volume, fadeTime)
}

// Check if ambient music is playing
export function isAmbientPlaying(): boolean {
  return audioSystem.isAmbientPlaying()
}
